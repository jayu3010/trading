...
let tableData = [];
const column = ['Sl. No', 'Name', 'Mobile Number', 'Date & Time'];
...
const PreferenceSharesEnquiry = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios
            .get(`http://43.204.210.119/api/request/preference`)
            .then((response) => {
                setLoading(false);
    ...
    return (
        <>
            <div>
                <MainCard title="Preference Shares Enquiry">
                    <SubCard>
                        {enquiry.length <= 0 ? (
                            <Box
                                sx={{
...
export default PreferenceSharesEnquiry;
